...
func (c *RoleBinding) Delete(namespace, name string, gracePeriodSeconds *int64) error {
	logs.Warning(map[string]interface{}{"名称": name, "命名空间": namespace}, "删除RoleBinding")
	...
	err := c.InstanceInterface.RoleBindings(namespace).Delete(context.TODO(), name, deleteOption)
	return err
}
...
func (c *RoleBinding) Update(namespace string) error {
	logs.Warning(map[string]interface{}{"名称": c.Item.Name, "命名空间": namespace}, "更新RoleBinding")
	c.Item.ObjectMeta.ResourceVersion = ""
	_, err := c.InstanceInterface.RoleBindings(namespace).Update(context.TODO(), c.Item, v1.UpdateOptions{})
	return err
}

// 获取资源列表
func (c *RoleBinding) List(namespace, labelSelector, fieldSelector string) (items interface{}, err error) {
	logs.Info(map[string]interface{}{"命名空间": namespace}, "查询RoleBinding列表")
	// 有可能是根据查询条件进行查询
...
func (c *RoleBinding) Get(namespace, name string) (item interface{}, err error) {
	logs.Info(map[string]interface{}{"名称": name, "命名空间": namespace}, "查询RoleBinding详情")
