...
func (c *Deployment) Delete(namespace, name string, gracePeriodSeconds *int64) error {
	...
	err := c.InstanceInterface.Deployments(namespace).Delete(context.TODO(), name, deleteOption)
	return err
}
...
func (c *Deployment) Get(namespace, name string) (item interface{}, err error) {
	logs.Info(map[string]interface{}{"名称": name, "命名空间": namespace}, "查询Deployment详情")
