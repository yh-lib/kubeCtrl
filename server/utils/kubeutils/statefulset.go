...
func (c *StatefulSet) Create(namespace string) error {
	logs.Info(map[string]interface{}{"名称": c.Item.Name, "命名空间": namespace}, "创建StatefulSet")
...
func (c *StatefulSet) Delete(namespace, name string, gracePeriodSeconds *int64) error {
	logs.Warning(map[string]interface{}{"名称": name, "命名空间": namespace}, "删除StatefulSet")
	...
	err := c.InstanceInterface.StatefulSets(namespace).Delete(context.TODO(), name, deleteOption)
	return err
}
...
func (c *StatefulSet) Update(namespace string) error {
	logs.Warning(map[string]interface{}{"名称": c.Item.Name, "命名空间": namespace}, "更新StatefulSet")
